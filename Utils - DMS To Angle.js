...
patch.inputs = [new PatchInput("DMS (String)", types.STRING, 0)];

patch.outputs = [new PatchOutput("Degree", types.NUMBER)];

// Add your logic in this function.
patch.evaluate = function () {
  patch.outputs[0].value = dmsToDecimalDegrees(patch.inputs[0].value);
};
...
function dmsToDecimalDegrees(dmsString) {
  ...
  // Adjust the sign based on the presence of a negative sign in the original string
  return dmsString.includes("-") ? -decimalDegrees : decimalDegrees;
}
